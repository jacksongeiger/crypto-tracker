...
- [ ] Register a free FRED API key at <https://fredaccount.stlouisfed.org/apikey> and set `FRED_API_KEY` in `backend/.env` to enable the macro skill. Without it the skill exits 0 with a skip message and synthesis runs without macro signals.
